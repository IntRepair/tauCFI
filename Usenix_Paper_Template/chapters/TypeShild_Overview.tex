...
%       \path[blue] (0,-0.25) node{0} (-0.25,0) node{0};
      \path[blue] (0.25,-0.25) node{1}  (-0.25, 0.25) node{1};
      \path[blue] (0.75,-0.25) node{2}  (-0.25, 0.75) node{2};
      \path[blue] (1.25,-0.25) node{4}  (-0.25, 1.25) node{4};
      \path[blue] (1.75,-0.25) node{8}  (-0.25, 1.75) node{8};
      \path[blue] (2.25,-0.25) node{16} (-0.25, 2.25) node{16};
      \path[blue] (2.75,-0.25) node{32} (-0.25, 2.75) node{32};
...
%       \fill (3.25, 0 + 0.25) circle(4pt);
    ...
    \begin{scope}[xshift=3.25cm, yshift=1.5cm]
     \node[]    (q_1) {$\wedge$};
    \end{scope}
    
    \begin{scope}[xshift=5.3cm, yshift=3.2cm]
     \node[]    (q_1) {$M2$};
    \end{scope}
    
    \begin{scope}[xshift=9cm, yshift=3.2cm]
     \node[]    (q_1) {$M3$};
    \end{scope}
...
The $X$ and $Y$ axis of matrices $M1, M2$ and $M3$ represent function parameter bit-widths.
Note that our type policy performs an $\wedge$ (i.e., logical and) operation
between each entry in $M1$ and $M2$. 
...
An arrow (\textit{i.e.,} in total 8) represents an indirect control flow transfer from a callsite to a calltarget. 
Note that~\cite{veen:typearmor} would allow all 8 illegal control flow transfers whereas \textsc{TypeSchield} would forbid all of them.}
