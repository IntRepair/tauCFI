...
relies on a precise construction of both the callee prototypes and call site signatures
and than uses this information to enforce that each call site targets matching functions 
...
which further reduces the possible target set of callees for each call site.
Our novel policy is based on the insight that if the binary adhere to the standard calling convention
for indirect calls, undefined arguments at the call site are not used by
...
plocies, often allowing controll transfers from any valid call site 
...
In contrast, \textsc{TypeShield} matches up indirect call sites with a more precise
...
and liveness analysis at indirect call sites to approximate call site signatures. This 
