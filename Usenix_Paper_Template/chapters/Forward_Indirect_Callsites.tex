...
in C++. 
Polymorphism in C++ allows accessing different types of objects 
...
callsites which are exploiter by calling illegitimate vTable entries (functions)
with varying number and type of parameters, \textit{x)}.
More precisely, 
\textit{1)} load-time enforcement: as calling each indirect callsite (callee) requires 
...
that belong to an indirect callsite.
...
The indirect callsite (line 17) can be exploited 
...
In order to perform the COOP attack the attacker crafts a C++ program containing an array buffer holding 
...
The crafted buffer will be called instead of the real code in order to call different functions
...
detection of the object types (base class), range of vTables (longest vTable inheritance path for a particular callsite)
