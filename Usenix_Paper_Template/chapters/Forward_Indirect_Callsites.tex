...
and so on up to 0 parameters. This holds because TypeArmor allows more parameters provided than consumed by the calltarget.
Than $c = 2100 = 600 + 500 + 400 + 300 + 200 + 100 \ iff \ x=y=z=t=p=v=100$. 
Note that $x=100$ is feasible under realistic conditions in large applications (i.e., Google Chrome, Firefox). 
Next $2100$ is added to $7^{6}$. 
Thus for a single call site providing $p=6$ parameters TypeArmor allows in total $7^{6} + 2100 = 1197496$ call targets for each callsite.
This situation repeats for $p=5$ where we get $7^{5} + (1500 = 500 + 400 + 300 + 200 + 100 \ iff \ x=y=z=t=p=v=100) \ = \ 18307$ 
allowed call target per call site. Note that this holds for the rest of $p \in [1, 4]$.
\end{itemize}
