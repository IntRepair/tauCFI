...
% \subsection{Checking Indirect Forward-Edge Calls in Practice}
...
greater or equal to $c$. Note that TypeArmor:

\begin{itemize}
\item \textit{Ideally} would allow for a single call site a set of call targets containing maximum $279936$ possibilities if we consider the maximum value of provided parameters to be
$p$=6 due to $p \in [0, 5]$ possible provided parameters. Consider 7 parameter types $t$: $bool, \ char, int, \ float, \ double, \ void, \ and \ wchar\_t$. Thus we obtain $6^{7}=279936$.
Note that in reality $t$ is even larger  
since there are many types of parameters in C++. 
The complete list of fundamental types in C++ is 20 not including data structures or object types.
Thus, all these call targets would be allowed by TypeArmor. Also, note that all other call sites having more 
than 6 parameters would be not checked by TypeArmor during runtime.

\item \textit{Actually} allows more than $p^{t}$ call targets per call site.
...
$y$ is the sum of all call targets consuming 5 parameters and so on up to $v$. 
The value of $c = 2100 = 600 + 500 + 400 + 300 + 200 + 100 \ iff \ x=y=z=t=p=v=100$. We consider 100 a realistic number for sufficiently
large C++ programs. Note that $2100$ is added to the previously value of $6^{7}$. 
Thus for a single call site providing $p=6$ parameters TypeArmor allows in total $6^{7}+2100 = 282036$ call targets per call site.
This situation repeats for $p=5$ where we get $5^{7} + (1500 = 500 + 400 + 300 + 200 + 100 \ iff \ x=y=z=t=p=v=100) \ = \ 79625$ allowed call target per call site. 
This is also true for the rest of $p \in (0, 3]$.
\end{itemize}
Finally, in our opinion TypeArmor is too permissive and as response to this situation we present 
\textsc{TypeShield} which copes with the variable type state explosion due to different parameter types by considering an 
approximation (i.e., alias analysis and thus type analysis in binaries is undecidable~\cite{alias:undecidable}) of 
parameter types based on register wideness. Consequently, the allowed call target set per call site is drastically reduced.

% \subsection{Security Implications of Forbidden Indirect Calls}
