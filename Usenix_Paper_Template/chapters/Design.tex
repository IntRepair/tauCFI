...
Khedker et al.~ \cite{khedker2009data} defines live variable analysis on blocks, which we used to arrive the
algorithm depicted in Figure~\ref{alg:liveness} to compute the liveness state at the start of a basic block. 
...
The book~\cite{khedker2009data} defines reaching definition analysis on blocks, which we use to arrive at algorithm depicted in Figure~\ref{alg:reaching} to compute 
...
value is an address within the \texttt{.bss}, \texttt{.data} or \texttt{.rodata} section of the binary. If this is the case, we simply return a write access of 64-bits 
