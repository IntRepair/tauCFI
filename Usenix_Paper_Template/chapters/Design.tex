...
\begin{enumerate}
\item W represents write before read access
\item R represents read before write access
\item C represents the absence of access
\end{enumerate}
...
Our horizontal merge($merge\_h$) function is a simple pairwise combination of the given set of states, which are then combined with a union like operator with W preceeding R preceeding C.
%\begin{align}
%merge\_h(\{s\}) &= s\\
...
\begin{enumerate}
\item W represents write before read access
\item r8, r16, r32, r64 represents read before write access with 8, 16, 32, 64 bit wideness
\item C represents the absence of access
\end{enumerate}
%\begin{enumerate}
...
As there could be more than one read of a register before it is written, we might be interested in more than just the first occurrence of a write or read on a path. To allow this we allow our merge operations to also return the value RW, which represents the existence of both read and write access and then can use W as an end marker of sorts.
...
%\end{itemize}
Therefore our vertical merge operator conceptually intersects all read accesses along a path until the first write occurs ($merge\_v^{i}$). In any other case it behaves like the previously mentioned vertical merge function.
...
Our horizontal merge($merge\_h$) function is again a simple pairwise combination of the given set of states, which are then combined with a union like operator with W preceeding WR preceeding R preceeding C. Unless one side is W, read accesses are combined in such a way that always the higher one is chosen.
...
%\label{fig:asmvariadic}
%\end{figure}
>>>>>>> 430261ca46d7b450bcc1cf2f5a89c3cea22cba8a
...
\item Look at the predecessor of the xmm-passthrough block, which we call the entry block (in our case basic block %[\texttt{0x4222f0},\texttt{0x4222f2} [ )
