...
\begin{itemize}
 ...
 \item \textbf{RQ1:} How precise is \textit{TypeShield} in recovering parameter count and type information for callsites and calltargets from a given binary?

 \item \textbf{RQ2:} How effective is \textit{TypeShield} in restricting the possible number of calltargets per callsite?

 \item \textbf{RQ3:} What is the performance overhead introduced by \textit{TypeShield}?
 \todo[inline]{address this}
...
\subsubsection{Precision Calltarget Classification (\textit{count})}
\label{subsection:typeshieldprecision}
...
\textbf{Experiment Setup.} We conducted the same three experiments as before but with the initial set of calltargets filtered by our implementation of address taken analysis:
\begin{itemize}
...
\textbf{Results.} First of all we observed an overall reduction of the geometric mean of overall available calltargets to 64\% before our policies were applied. Notable outliers are memcached, which had a 90\% reduction of available calltargets and vsftpd, which even achieved a 97\% reduction in available calltargets.
\begin{itemize}
...
\subsection{RQ4: Runtime Performance Overhead}
...
\subsection{RQ5: Instrumentation Overhead}
...
\subsection{RQ6: Security Level of \textit{TypeShield}}
