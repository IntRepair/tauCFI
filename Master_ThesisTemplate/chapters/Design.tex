...
In the design of Typeshield, we cover various aspects, first of all we introduce the details of the \emph{count} policy - as introduced by \cite{veen:typearmor} and the new \emph{type} policy. Then we need to implement the analysis for calltargets callsites for the the respective policy. For further restriction capabilities we introduce a version of address taken analysis based on \cite{mingwei:sekar}. 
...
\caption{\emph{count} policy classification schema}
