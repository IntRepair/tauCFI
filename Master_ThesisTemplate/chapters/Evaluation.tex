...
\end{enumerate}
The problematic column is the clang miss column, as these might indicate problems with \textit{TypeShield}, however as these numbers are relatively low (below 1\%) throughout optimization levels, with only node showing a significant higher value than the rest of around 1.6\%. The column labeled tool miss lists higher numbers, however these are of no real concern to us, as our ground truth pass possibly collects more data. The data of all source files used during the compilation of our testtargets is collected, which might generate more than one binary and therefore not necessary all sourcefiles are used for our testtarget.

Considering this, we can safely state that our structural matching between ground truth and \textit{TypeShield} regarding calltargets is nearly perfect (\textgreater 98\%)
...
\resizebox{\textwidth}{!}{
	...
	\toprule
	\multicolumn{1}{c}{\bfseries O0} & \multicolumn{1}{c}{\bfseries AT} & \multicolumn{4}{c}{\bfseries COUNT safe} & \multicolumn{4}{c}{\bfseries COUNT prec}\\
...
\resizebox{\textwidth}{!}{
	...
	\toprule
	\multicolumn{1}{c}{\bfseries O0} & \multicolumn{1}{c}{\bfseries AT} & \multicolumn{4}{c}{\bfseries TYPE safe} & \multicolumn{4}{c}{\bfseries TYPE prec}\\
