...
$\texttt{merge\_v} : \mathcal{S}^\mathcal{L} \times \mathcal{S}^\mathcal{L} \mapsto \mathcal{S}^\mathcal{L}$, which describes how to merge the current state with the following state change.

$\texttt{merge\_h} : \mathcal{P}(\mathcal{S}^\mathcal{L}) \mapsto \mathcal{S}^\mathcal{L}$, which describes how to merge a set of states resulting from several paths.
...
\begin{algorithm}[h!]
  ...
  \Input{The basic block to be analyzed - block : $\texttt{INSTR}^*$}
	...
	blocks = \texttt{succ(block)}                                        \Comment*[r]{Get successors(succ.)}
...
To implement the \emph{count} policy, we only need a coarse representation of the state of one register, thus we use the same representation 
...
\subsubsection{Ignoring Reads} When one instruction writes and reads a register at the same time we give the read access precedence, however, there are exceptions 
...
An assignment to a variable is a reaching definition after the execution of a set of instruction if that variable still exists in at least one possible execution path. If applied to a callsite, this calculates the values that are provided by this callsite to the function it then invokes. We based Algorithm~\ref{alg:reaching}  on the reaching defintion analysis presented in Khedker~\textit{et al.}~\cite{khedker2009data}, which basically is a reverse depth-first traversal of basic blocks. For customization, we rely on the implementation of several functions($\mathcal{S}^\mathcal{R}$ is the set of possible register states depending on the specific reaching definition implemention):

$\texttt{merge\_v} : \mathcal{S}^\mathcal{R} \times \mathcal{S}^\mathcal{R} \mapsto \mathcal{S}^\mathcal{R}$, which descibes how to merge the current state with teh following state change.

$\texttt{merge\_h} : \mathcal{P}(\mathcal{S}^\mathcal{R}) \mapsto \mathcal{S}^\mathcal{R}$, which descibes how to merge a set of states resulting from several paths.

$\texttt{analyze\_instr} : \texttt{INSTR} \mapsto \mathcal{S}^\mathcal{R}$, which calculates the state state change that occurs due to the given instruction.

$\texttt{pred} : \texttt{INSTR}^* \mapsto \mathcal{P}(\texttt{INSTR}^*)$, which calculates the predecessors of the given block.
...
\begin{algorithm}[!ht]
	...
	states = \{\}                                \Comment*[r]{Set of predecessor states}
	
	blocks = pred(block)                         \Comment*[r]{Get predecessors(pred.)}
	...
	state' = merge\_h (states)                   \Comment*[r]{Merge pred. states}
...
\subsubsection{Provided Parameter Wideness}
