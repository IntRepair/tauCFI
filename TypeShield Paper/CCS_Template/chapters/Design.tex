...
An instruction $i \in \texttt{INSTR}$ can non-exclusively perform two kinds of operations on any number of existing registers:\footnote{There are registers that can directly access the higher 
...
As previously mentioned there are 4 types of reading and writing access each. Therefore our set of possible types for parameters is $\texttt{TYPE} = \{64, 32, 16, 8, 0\}$; 0 models the absence of a parameter. Since SystemV ABI specifies 6 registers as parameter holding 
...
callsite $cs$ to be able to call the calltarget $ct$, if for each parameter of $ct$ the corresponding parameter of $cs$ is not smaller when comparing the wideness.
