...
\resizebox{\columnwidth}{!}{
	...
	{\textit{\csvcolii} & \csvcoliii & \csvcoliv & \csvcolv & \csvcolvi & \csvcolvii}% specify your coloumns here
    	\end{tabular}}
%     	}
	\caption {The medians of calltargets per callsite for different restrictions methods. Note that the smaller the numbers, the better the method. AT is the approach to only allow calltargets that are address taken. IFCC is a compiler based approach and only here as a reference of what is possible. TypeArmor and TypeShield on the other hand are binary based tools. We can see that our type based tool on average reduces the number of calltargets by 37\% when compared to the AT method and by 12\% when comparing with TypeArmor.
}
