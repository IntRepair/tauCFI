...
\begin{center}
 \resizebox{2\columnwidth}{!}{
	...
	\caption {The results regarding the restriction of allowed callsites per calltarget for several test series on various targets compiled under optimization O2. Note that the basic restriction to address taken only calltargets (see column AT) is present for each other series. The label $count^*$ denotes the best possible reduction using our \emph{count} policy based on the ground truth collected by our clang/LLVM pass, while $count$ denotes the results of our implementation of the \emph{count} policy derived from the binaries. The same applies to $type*$ and $type$ regarding the \emph{type} policy. A lesser number of calltargets per calltsite indicates a better series. We can see that \emph{type} policy is superior to the \emph{count} policy, as it allows for a stronger reduction of allowed calltargets. We provided the median and the pair of mean and standard deviation to allow for comparison of other tools.
% 	\textcolor{red}{TODO-add more description in order to indicate the advantage of our tool. What geomean values are good, low or hig? What is the main thing which can be observed if 
% 	looking at this table?}
        }
...
\resizebox{\columnwidth}{!}{
	...
	{\textit{\csvcolii} & \csvcoliii & \csvcoliv & \csvcolv & \csvcolvi & \csvcolvii}% specify your coloumns here
    	\end{tabular}}
%     	}
	\caption {The medians of calltargets per callsite for different restrictions methods. Note that the smaller the numbers, the better the method. AT is the approach to only allow calltargets that are address taken. IFCC is a compiler based approach and only here as a reference of what is possible. TypeArmor and TypeShield on the other hand are binary based tools. We can see that our type based tool on average reduces the number of calltargets by 33\% when compared to the AT method and by 6\% when comparing with TypeArmor. When looking at Node.js we reach 19\% reduction compared to TypeArmor and when looking at PostgreSQL, we even achieve a reduction of about 23\%.
}
