...
\end{table}
and the number and ratio of problematic classifications, which in the case of calltargets refers to overestimations and in case of callsites refers to underestimations. The results are depicted in Table \ref{tbl:precisionTYPE}.
...
The obtained geomean runtime overhead is under 1.1\% performance drop when instrumenting using Dyninst. 
One reasons for the performance drop includes cache misses introduced by jumping between the old and the new executable section 
...
Finally, our runtime overahead is lower (2\%) than state-of-the-art tools (around 3\%), 
