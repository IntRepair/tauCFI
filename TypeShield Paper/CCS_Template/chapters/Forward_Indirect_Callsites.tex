\section{Forbidden Forward Calls Exposed}
...
\subsection{Exploiting Object Dispatches}
...
Each \texttt{vfgadget} \circled{1} can be called in several ways, see ML-G, REC-G and UNR-G in 
Figure~\ref{Code example used to illustrate how a COOP loop gadget works}.
...
We build our own \emph{count} policy which essentially resembles the policy introduced by TypeArmor \cite{veen:typearmor} and
...
% 
% We do not agree with this claim and furthermore we believe that their callsite vs. call target set enforcing policy is too permissive and thus many potential indirect forward edge based 
...
% the aforementioned callsite overestimation and call target underestimation, thus all possible numerical mismatches are allowed by their policy as long as $p$ is 
% greater or equal to $c$.
% 
% \begin{itemize}
% \item TypeArmor \textbf{\textit{ideally}} would allow for a single callsite a set of call targets containing maximum $117649$ possibilities if we 
...
% Thus we obtain $t^{p}=7^{6}=117649$ allowed call targets per callsite if the TypeArmor is in place.
...
% 
% \item TypeArmor \textbf{\textit{actually}} allows more than $t^{p}$ call targets per callsite.
...
% allowed call target per callsite. Note that this holds for $p \in [1, 4]$ too.
% \end{itemize}
% 
...
% parameter types based on register wideness. Consequently, the allowed call target set for each callsite is drastically reduced.
% 
