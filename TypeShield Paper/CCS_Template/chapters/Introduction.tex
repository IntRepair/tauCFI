...
\begin{tikzpicture}
\hspace{-.5cm}
  \node (img)  {\includegraphics[scale=0.75]{figures/distri.pdf}};
...
% \textbf{Problem.}
The performance benefit of late binding comes with high security implications (\textit{i.e.,} 701 arbitrary code executions~\cite{NVD:ACE} reported by US NIST NVD, see Figure~\ref{ace:nvd:statistics}).
...
%    \begin{picture}(10,4)
% %    \centering
%      \put(1.8, .69){\includegraphics[width=.27\textwidth]{figures/loop.pdf}}
%      \put(.2, 2){\usebox{\firstlisting}}
%    \end{picture}
