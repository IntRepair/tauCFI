...
\centering
   \includegraphics[width=.88\textwidth]{figures/overview.pdf}
    ...
    The program binary is analyzed by \textsc{TypeShield} and the calltargets and callsite analysis are performed for determining how many parameters are 
    provided, how many are consumed and their register wideness.
    After this step labels are inserted at each previously identified callsite and at each calltargets. Black dots above are allowed to call only legitimate red dots.
    ...
    address where the function return wants to dispatch to is contained in the legitimate set for each particular return site.
    Green dots above are allowed to call only legitimate blue dots.}
