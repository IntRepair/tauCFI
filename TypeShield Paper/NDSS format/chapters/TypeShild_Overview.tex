...
In this section, we introduce the threat model used in this word in~\cref{Adversary Model}, and in
in~\cref{Invariants for Targets and Callsites} we present the invariants for calltargets and callsites, while 
in~\cref{TypeShield Policy Mechanism} we present our function parameter type aware policy.
...
\centering
   \includegraphics[width=.88\textwidth]{figures/overview.pdf}
    ...
    The original program binary is analyzed (left hand side) by \textsc{TypeShield} and the calltargets and callsite analysis are performed for determining how many parameters are 
    ...
    The enforced policy is schematically represented by the black dots (addresses) above are allowed to call only legitimate red dots (addresses).
    Next for each function return the set of address sets which are next to each callsite that is allowed to call the function in first place are collected.
    This information is obtained trough the previous forward mapping.
    In this way we have for each function return site a set of legitimate addresses where the function return site is allowed to dispatch to.
    ...
    Green dots (addresses) above are allowed to call only legitimate blue dots (addresses).
    The result is a hardened program binary (right hand side).}
...
\end{definition}
Whereas the policy of \textsc{TypeShield} is as follows.
\begin{definition}
