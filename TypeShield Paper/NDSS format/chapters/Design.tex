...
In case  there is a write operation on the \texttt{RAX} register than we know that the function return is non-void 
and thus provides a return value.
...
In case there is a read before write on the \texttt{RAX} register than
\textsc{TypeShield} infers that the callsite is non-void and thus expects a variable to be provided 
...
\subsection{Enforcing Our Forward Edge Policy}
	...
	\Fn{\texttt{backwardAddressMapping}($fMap$) : $rMap$}
	{
 	...
 	\ForEach{callsite $\in$ $fMap$}{  
 	  ...
 	  $calltargetSet$ = $getCalltargetSet(callsite, fMap)$\;
        ...
        \Comment*[l]{return the backward edge addresses mappings}
...
The return target address set is determined by incrementing the current address of
