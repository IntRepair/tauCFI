...
in~\cref{section:instructionanalysis} the theory and definitions for our instructions analysis based on register states,  
in~\cref{section:calltargetanalysis} we present the design of our calltarget analysis, while
in~\cref{section:callsiteanalysis} we present the design of our callsite analysis.
Finally,  
in~\cref{Enforcing The Forward Edge Policy} we briefly present our forward edge policy instrumentation strategy, and 
...
\draw[-triangle 45, thick] (-0.5,0) -- node[sloped, anchor=center, above] {\LARGE{growing required bits/parameter}} (-0.5,7);
\draw[-triangle 45, thick] (6.5,7) -- node[sloped, anchor=center, above]  {\LARGE{growing provided bits/parameter}} (6.5,0);
...
\caption{Callsite \& calltargets {count} policy classification schema.}
...
also on the parameter type. We use the register width as parameter type. As previously mentioned, there are 4 types 
...
parameter. Since Itanium C++ ABI specifies 6 registers (i.e., \texttt{rdi}, \texttt{rsi}, \texttt{rdx}, \texttt{rcx}, \texttt{r8}, and \texttt{r9}) as parameter passing registers, we classify our callsites and calltargets into $\texttt{TYPE}^6$. Similar to
the policy of \texttt{TypeArmor}, we allow overestimations of callsites and underestimations of calltargets, however, on the parameter types as well. Therefore, for a 
...
For customization, we rely on the implementation of several functions which we will present next. $\mathcal{S}^\mathcal{L}$ is the set of possible register states depending on the specific 
liveness implementation of the following operations.
...
%merge\_h(\{s\} \cup s') &= s \circ merge\_h(s')
%\end{align}
Our horizontal \textit{merge($merge\_h$)} function is a pairwise combination of the given set of states, which are then combined with a union-like operator 
...
\caption{Assembly code of the \texttt{make\_cmd} function which was compiled with Clang -O2 flag, which has a variadic parameter list, shaded gray above.}
\label{fig:asmvariadic}
\end{figure}
...
(2) we look at the predecessor of the xmm-passthrough block, which we call the entry block (in our case basic block %[\texttt{0x4222f0},\texttt{0x4222f2} [ )
...
(1) \texttt{xor \%rax, \%rax} is the first obvious scenario, as it will always result in \texttt{\%rax} holding the value 0,
(2) \texttt{sub \%rax, \%rax} is probably the next scenario, as it results in \texttt{\%rax} also holding the value 0, and
...
underestimations are not be permitted. For this purpose we employ a customizable modified reaching definition algorithm, 
which we will show first. Furthermore, we will highlight some corner cases we encountered.(merge vertically block states)
...
depending on the specific reaching definition implementation of the following operations.
...
For implementing the our {count} policy, we need a coarse representation of the state of one register, thus we use the following representation.
...
In order to implement our {type} policy, we need a finer representation of the states of one register, thus we consider:
(1) $T$ represents a trashed register,
...
preceding $S$ and $S$ preceding $U$. When both states are set, we pick the higher one.
...
\subsection{Enforcing The Forward Edge Policy}
...
is used to define if the function is expecting a \texttt{void} return type of not. All this information are written in labels before
each callsite and calltarget. During runtime before each callsite the these labels are compared by performing a xor operations between 
the bits contained in the previously mentioned labels. In case the xor operation returns true
...
address and a return target address. 

This is determined by incrementing the current address of
the callsite for which the forward edge propagation algorithm is applied 
by one (i.e., recall the caller callee calling convention).
...
Note that the function boundary address (i.e., retn) was detected by a linear search from the beginning of 
...
function boundaries (e.g.,~\cite{function:boundary}) exist, but we leave this as future work.
