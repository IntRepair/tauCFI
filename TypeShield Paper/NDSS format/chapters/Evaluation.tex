...
(4) uses a restricted number of labels (\text{i.e.,} each function called from inside a function will get the same label. This label will be 
...
More precisely, we have a 1 to $N_{0},..,N_{m}$ (legitimate call target returns) number of compares and a 1 to $N_{max}-N_{min}$ ($N_{max}$ is 
the maximum address from  $N_{0},..,N_{m}$ and $N_{min}$ is the minimum address from  $N_{0},..,N_{m}$) for each function return.
...
\textit{min} addresses for all calltarget return sites contained in the MySQL and Node.js programs and also the average count of return addresses for all 
functions return sites.
...
\begin{threeparttable}
 \begin{tabular}{ l  c  p{2.5cm}  } 
  ...
  All Backward 
  edge \\ violating 
  attacks            & \checkmark (BP)  & (1)\tnote{a} (2)\tnote{b}.
...
by deploying only the forward or the backward policy. 
Note that FP means forward policy while BP means backward policy. In summary we note that all forward-edge based attacks 
and backward edge attacks can be successfully mitigated by \textsc{TypeShield} as long these attacks are not 
aware of the policy in place and thus can be selectively use gadgets which have their start address in the 
