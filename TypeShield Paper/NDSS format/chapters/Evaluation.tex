...
%  \item[
 \textbf{RQ7:} Is shadow-stack \textbf{better} than \textsc{TypeShield}? (\cref{Comparison with SafeStack})\vspace{-.2cm}
 
%  \item[
 \textbf{RQ8:} Are other tools \textbf{better} than \textsc{TypeShield}? (\cref{RQ5: Is TypeShield better than other tools?})\vspace{-.2cm}
 
 \textbf{RQ9:} Is \textsc{TypeShield} \textbf{effective} against COOP? (\cref{Effectiveness Against COOP})\vspace{-.2cm}
 
 \textbf{RQ10:} Which \textbf{attacks} mitigates \textsc{TypeShield}? (\cref{Mitigation of Advanced Code-Reuse Attacks})\vspace{-.2cm}
...
to several SPEC CPU2006 benchmarks.
...
\hspace{-.3cm}
  \begin{minipage}[b]{1.0\linewidth}
    \centering
    \resizebox{.8\columnwidth}{!}{\includesvg{svgs/postgresqlO2}}
%     \vspace{-.7cm}
...
\subsection{Backward-edge Comparison with Shadow-Stack (RQ7)}
...
The only binary
based approach is the one from Abadi et al.~\cite{abadi:cfi2}. This solution has:
...
that our policy can eliminate 99.98\% (100 - 0.012) of the addresses which an attacker can use for his attack inside the binary.
...
\subsection{Effectiveness Against COOP (RQ9)}
...
\subsection{Mitigation of Advanced Code-Reuse Attacks (RQ10)}
