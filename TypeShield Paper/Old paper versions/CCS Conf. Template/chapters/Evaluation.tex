...
ftp server applications \textit{Vsftpd} (v.1.1.0) and \textit{Proftpd} (v.1.3.3), the two database server 
...
\begin{itemize}[leftmargin=.12in]
 ...
 \item \textbf{RQ6:} Which \textbf{upper bounds} can \textsc{TypeShield} enforce? (\cref{RQ6:TyperArmor's Imprecise Parameter-Count Policy})
...
\resizebox{\columnwidth}{!}{
	...
	\bfseries Target & match & Clang miss &  TS miss &  match & Clang miss & TS miss% specify table head
	\\\midrule
...
%     	}
	...
	denotes elements not found in the data-set of the Clang/LLVM pass. The label TS denotes elements not found in the data-set of \textsc{TypeShield}. 
...
(3) TS miss: Number of calltargets that are found by our Clang/LLVM pass, but not by \textsc{TypeShield}.
...
(3) TS miss: Number of callsites that are discarded from the data set of our Clang/LLVM pass.
Both columns (Clang miss and TS miss) show a relatively low number of encountered misses. Therefore, we can 
...
\textbf{Results (Calltargets).}
The rate of perfect classification is in general over 86\% while for the calltargets it is over 70\%.
...
\textbf{Results (Callsites).} The rate of perfect classification is over 71\% while for the returns it over 88\%.
...
\textbf{Experiment Setup (Calltargets).} We used the union combination operator with an $analyze$ function that follow into occurring direct calls and a vertical merge and that intersects all reads until the first write.

\textbf{Results (Calltargets).} The rate of perfect calltargets classification is over 77\% while for the returns is over 77\%.

\textbf{Experiment Setup (Callsites).} We used the union combination operator with an $analyze$ function that does not follow into occurring direct calls while relying on a backward inter-procedural analysis.

\textbf{Results (Callsites).} The rate of perfect classification is in general over 66\% while for the returns is over 88\%.
...
\textbf{Results.}
(1) The theoretical limit of the \textit{count*} schema has a geometric mean of 129 possible calltargets, which is 12\% of the geometric mean of the total available calltargets (1074, see Table~\ref{tbl:precisionTYPE}), and
(2) The theoretical limit of the \textit{type*} schema has a geometric mean of 105 possible calltargets, which is 9.7\% of the geometric mean of the total available calltargets (1074, see Table~\ref{tbl:precisionTYPE}).
When compared, the theoretical limit of the \textit{type*} policy allows about 19\% less available calltargets in the geomean with Clang -O2 than the limit of the \textit{count*} policy.
...
\textbf{Results.}
(1) The \textit{count} schema has a geometric mean of 166 possible calltargets, which is around 15\% of the geometric mean of total available calltargets (1074, see Table~\ref{tbl:precisionTYPE}). 
This is around 28\% more than the theoretical limit of available calltargets per callsite, see \textit{count*}, and
(2)  The \textit{type} schema has a geometric mean of 144 possible calltargets, which is around 13\% of the geometric mean of total available calltargets (1074, see Table~\ref{tbl:precisionTYPE}). 
This is around 37\% more than the theoretical limit of available calltargets per callsite, see \textit{type*}.
When compared, our implementation of the \textit{type} policy allows around 21\% less available calltargets in the geomean with Clang -O2 than our implementation of the \textit{count} policy.
...
Figures~\ref{fig7}
depict the CDFs of the Postgresql program which was compiled with the Clang -O2 flag. 
...
Furthermore, we believe that their callsite vs. calltarget set enforcing policy is too permissive and thus
many illegitimate indirect forward edge based control flow transfers are possible. 

Consider the following example, where each callsite is
preparing, say, $p=4 \in [1, 6]$ parameters. Then TypeArmor policy would allow calltargets which consume the same number of parameters as
prepared, $c=4 \in [1, 6]$ or a lower value. Thus all possible numerical parameter mismatches are allowed by TypeArmors policy as long
...
Finally, as TypeArmor is too permissive (see also Figure H.1~\cite{vci:asiaccs}) we present \textsc{TypeShield} which 
can deal with the aforementioned 4 types of register wideness which can further help to reduce the the allowed
calltarget set per callsite.
