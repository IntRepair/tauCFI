...
Source code based tools which can protect against forward-edge violations such as: SafeDispatch~\cite{safedispatch:jang}, ShrinkWrap~\cite{haller:shrinkwrap}, VTI~\cite{bounov:interleaving}, 
...
%%Our idea
In this paper, we present \textsc{TypeShield}, a fine-grained CFI-complete (\textit{i.e.,} forward-edge and backward-edge protection) runtime 
...
%  \textbf{Novel forward and backward edge protection technique.} 
 We designed a novel fine-grained CFI technique for protecting forward-edges and backward-edges in stripped (\textit{i.e.,} no RTTI information) program binaries.
...
%  \textbf{Usable Open Source Prototype.} 
 ...
 for stripped program binaries. 
%  \textsc{TypeShield} can serve as platform for developing other binary based protection mechanisms.
...
%  Further, we show that our tool has a higher calltarget set per callsite reduction, thus 
