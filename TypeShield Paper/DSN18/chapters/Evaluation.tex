...
%  
%   \item[
%  \textbf{RQ10:}] Is shadow-stack \textbf{better} than \textsc{TypeShield}? (\cref{RQ10: Comparison with SafeStack})
% %  \vspace{.1cm}
%   
% \end{itemize}
...
% \hspace{-.27cm}
% \hspace{-.27cm}
% \textbf{RQ3:} What \textbf{security level} offers \textsc{TypeShield}? (\cref{RQ5: Security Analysis})
% \hspace{-.27cm}
% \textbf{RQ6:} Which \textbf{upper bounds} has \textsc{TypeShield}? (\cref{RQ6:TyperArmor's Imprecise Parameter-Count Policy})
% \hspace{-.27cm}
...
% \hspace{-.27cm}
% \textbf{RQ9:} What \textbf{binary blow-up} has \textsc{TypeShield}? (\cref{section:typeshieldoverheadinstrumentation})
...
\resizebox{\columnwidth}{!}{
	\begin{tabular}{l|r|r|p{1.9cm}|r|p{1.6cm}|p{1.9cm}}%
	\toprule
...
%     	}
	\caption {The quality of structural matching provided by our automated verify and test environment, 
...
% \vspace{-.5cm}
\end{table}
Table~\ref{tbl:matchingquality} depicts the results obtained w.r.t. the investigation of calltargets comparability and the callsites compatibility.
...
\textbf{Callsites.} While structural matching of calltargets is rather simple, matching callsites is more complex. Our tool can provide accurate addressing of 
...
% }
% 
% \caption {The results for classification of callsites and calltargets using our \textit{count} policy on the \texttt{-O2} optimization level, ground truth obtained by our Clang/LLVM pass.
% % 		\textcolor{red}{TODO-add more description in order to indicate the advantage of our tool. What geomean values are good, low or hig? What is the main thing which can be observed if 
% % 	        looking at this table?}
%                 }
...
% 
% \textbf{Calltargets.} For the first experiment we used an union combination operator with an $analyze$ function that follows into occurring direct calls.
% The results indicate a perfect classification of around 86\% while for the returns it is over 68\%.
% 
% \textbf{Callsites.} For the second experiment we used an union combination operator with an $analyze$ function that does not follow into occurring direct calls while relying on a backward inter-procedural analysis.
...
and a vertical merge and that
\begin{table*}[htbp!]
\begin{center}
 \resizebox{2.055\columnwidth}{!}{
...
%     \centering
%     \resizebox{.7\columnwidth}{!}{\includesvg{svgs/postgresqlO2}}
%     \vspace{-.4cm}
...
% 
% % \item 
% TypeArmor {\textit{actually}} allows more than $t^{p}$ calltargets per callsite. If we have $t=4$ integer types due to TypeArmors overestimation
...
% \subsection{Mitigation of Advanced CRAs (RQ5)}
...
% \vspace{-.3cm}
% \begin{tablenotes}
%   \item[a] Jump to an address $\notin$ in the $max - min$ address range.
%   \item[b] Jump to an address $\ne$ to one of the legitimate addresses.
%   \item[c] Jump to an address label $\ne$ with the calltarget return label.
%   \end{tablenotes}
%   \end{threeparttable}
% }
% \caption{Stopped CRAs, forward-edge policy (FP) and backward-edge policy (BP).}
% \label{Stopped attacks.}
% \vspace{-.5cm}
% \end{table}
% Table~\ref{Stopped attacks.} depicts several attacks that can be successfully stopped by \textsc{TypeShield}
...
\subsection{Forward-Edge Policy vs. Other Tools (RQ3)}
...
\begin{table}[h]
\resizebox{\columnwidth}{!}{
	\begin{tabular}{l|r|p{1.6cm}|r|p{1.5cm}|p{1.5cm}}%
	\toprule
	\bfseries \textbf{Target} & \textbf{IFCC}  & \textbf{TypeArmor (CFI+CFC)}  & \textbf{AT}  &  \textbf{TypeShield (count)} & \textbf{TypeShield (type)}% specify table head
	\\\midrule
...
% We compared our version of address taken analysis (AT), TypeArmor, \textsc{TypeShield} (count), \textsc{TypeShield} (type) and IFCC. 
% Interesting to note
% is that when comparing these values, one can see that we did not depicted a separation based on return type or the 
% CFC that TypeArmor introduced. 
% Therefore, when implementing those measures, we think that our solution would improve even 
...
%   \textit{geomean}& 160  & 155  & 1   & 1   & 1   & 1   & 1   \\\hline
%   MySql (REC-G)   & 289  & 273  & 10  & 2   & 3   & 0   & 1   \\
%   Node.js (REC-G) & 72   & 69   & 2   & 0   & 0   & 0   & 1   \\
%   \textit{geomean}& 144  & 137  & 4   & 1   & 1   & 1   & 1   \\\hline
% \end{tabular}
% }
...
% the original shadow-stack implementation from Abadi \textit{et al.}~\cite{abadi:cfi2}(binary-based), 
% SafeStack~\cite{volodymyr:cpi} (compiler based and bypassed~\cite{safestack:bypassing}), 
% MoCFI~\cite{mcfi:niu} (compiler based), HAFIX~\cite{hafix} (compiler based and bypassed~\cite{hafix:bypass}), and 
% also PathArmor~\cite{veen:cfi} (similarly by-passable as kBouncer, ROPecker, and ROPGuard~\cite{schuster:raid} 
% as it relies on a capacity restricted LBR register.)
...
\subsection{Runtime Overhead (RQ5)}
...
%         ymin=-2,
        ...
        ylabel={},
        xlabel={},
        symbolic x coords={bzip2, gcc, sphinx3, gobmk, perlbench, lbmc, milc, dealII, namd, soplex, mcf,povray, hmmer, sjeng, libquantum, h264ref, omnetpp, astar, xalancbmk, geomean},
        xtick=data,
        width=9cm,
        height=6.5cm,
%         ymajorgrids=true,
%         ytick={0.9,1, 1.01, 1.02, 1.04, 1.06, 1.08, 1.1, 1.2},
        ytick = data,
...
%         y label style={at={(0.1,0.5)}},
        ...
        nodes near coords align={vertical},
        cycle list={red, green}
      ]
      \addplot[darkgray, fill=darkgray] coordinates {
        (bzip2, 0.2) (gcc, 0.3) (sphinx3, 0.5) (gobmk, 0.2) (perlbench, 0.4) (lbmc, 0.23) (milc, 0.4) (dealII, 0.44) (namd, 0.29)  (soplex, 0.29) (mcf, 0.41) (povray, 0.43) (hmmer, 0.39) (sjeng, 0.46) (libquantum, 0.43) (h264ref, 0.53) (omnetpp, 0.59) (astar, 0.62) (xalancbmk, 0.44) (geomean, 0.43)
      };
      \addplot[black, fill=black] coordinates {
        (bzip2, 0.25) (gcc, 0.35) (sphinx3, 0.56) (gobmk, 0.29) (perlbench, 0.45) (lbmc, 0.33) (milc, 0.45) (dealII, 0.66) (namd, 0.40) (soplex, 0.41) (mcf, 0.56) (povray, 0.48) (hmmer, 0.39) (sjeng, 0.47) (libquantum, 0.38) (h264ref, 0.51) (omnetpp, 0.42) (astar, 0.45) (xalancbmk, 0.34) (geomean, 0.45)
      };
      \legend{Vanila Benchmark, \textsc{TypeShield}}
    \end{axis}
  \end{tikzpicture}
  \vspace{-.5cm}
  \caption{Benchmark run time normalized against the baseline for the SPEC CPU2006 benchmarks.}
  \label{overhead:spec:cfi}
\end{figure}\vspace{-.3cm}
\paul{add rest of the spec benchmarks.}
Figure~\ref{overhead:spec:cfi} depicts the runtime overhead obtained by applying \textsc{TypeShield} (forward-edge policy (parameter count and register type) and backward-edge policy)
...
% \subsection{Instrumentation Overhead (RQ9)}
