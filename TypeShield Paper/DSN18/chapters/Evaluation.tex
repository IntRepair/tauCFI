...
\subsection{Runtime Overhead (RQ3)}
...
\begin{figure}[H]
  \begin{tikzpicture}
    \begin{axis}[
        ...
        xlabel={},
%         symbolic x coords={444.namd,447.dealII,450.soplex,453.povray,471.omnetpp,473.astar,average,geomean},
        symbolic x coords={namd,xalancbmk,dealII,soplex,povray,omnetpp,astar,average,geomean},
        xtick=data,
        width=9.5cm,
        height=6cm,
      ...
      \addplot[lightgray, fill=lightgray] coordinates {
        (namd,{0.4}) (xalancbmk,4.49) (dealII,0.47)
        (soplex,5.03) (povray,2.34) (omnetpp, 0.68) (astar,0.90) (average,2.04) (geomean,1.29)
      };
      \addplot[black, fill=black] coordinates {
        (namd,0.0) (xalancbmk,1.78) (dealII,0.94)
        (soplex,2.07) (povray,1.57) (omnetpp,0.34) (astar,0.30) (average,1.0) (geomean,0.92)
      };
      \legend{vanila, \textsc{TypeShield}}
    \end{axis}
  \end{tikzpicture}
  \caption{SPEC CPU2006 benchmark runtime with no instrumentation (shaded gray), and with \textsc{TypeShield} (shaded black).}
  \label{overhead:spec:cfi}
\end{figure}

Figure~\ref{fig:awesome_image} depicts the runtime overhead obtained by applying \textsc{TypeShield} (forward-edge policy (parameter count and type mode) and backward-edge policy (fast mode))
...
The obtained runtime overhead is around 3\% (geomean) when instrumenting the binary with DynInst. 
...
However, the runtime overhead of \textsc{TypeShield} (3\%) is comparable with other forward-edge protection
tools such as: TypeArmor (3\%), VCI~\cite{vci:asiaccs} (7.79\% overall and 10.49\% on only the SPEC CPU2006 programs), 
vfGuard~\cite{vfuard:aravind} (10\% - 18.7\%), T-VIP~\cite{gawlik} (0.6\% - 103\%), 
...
% \hspace{-.3cm}
  \begin{minipage}[b]{1.0\linewidth}
    \centering
    \resizebox{.9\columnwidth}{!}{\includesvg{svgs/postgresqlO2}}
    \vspace{-.4cm}
...
\subsection{Forward-Edge Policy vs. Other Tools (RQ7)}
...
\begin{table}[h]
\resizebox{\columnwidth}{!}{
	\begin{tabular}{l|r|p{1.5cm}|r|p{1.5cm}|p{1.5cm}}%
	\toprule
...
% the original shadow-stack implementation from Abadi et al.~\cite{abadi:cfi2}(binary-based), 
...
The safe stack implementation of Abadi et al.~\cite{abadi:cfi2} has the highest
