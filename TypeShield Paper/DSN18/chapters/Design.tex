...
We use the register width of the function parameter in order to infer the type information. There are 4 types 
...
occurs, $merge\_v^{i}$. In any other case, it behaves like the vertical merge function.
...
\textsc{TypeShield} looks at the callsite if there is a read before write on the \texttt{RAX} register. 
In this is the case,
...
the bits contained in the previously mentioned labels. In case the xor operation returns false, then the transfer is allowed, else the program execution is terminated.
...
% \textbf{Fast mode.} 
Based on the $rMap$, before each AT function returns a randomly generated label (\textit{i.e.,} the value 7232943 will be loaded through one level of indirection) 
