...
Figure~\ref{Code example used to illustrate how a COOP loop gadget works}.
The object dispatch (see Figure~\ref{Code example used to illustrate how a COOP loop gadget works} line 17) can be exploited 
by passing a varying number of parameters and types on each object contained in the used array.
In this way different targets contained in the program (1) virtual table (vTable) hierarchy (contains only virtual members), 
(2) class hierarchy (contains both virtual and non-virtual members) and (or) the whole program can be called. 
For example attacker can call in this way in the:
\textit{1)} class hierarchy of the whole program,
\textit{2)} class hierarchy containing only legitimate targets for this callsite,
\textit{3)} virtual table hierarchy of the whole program,
\textit{4)} virtual table hierarchy containing only legitimate targets for this callsite,
\textit{5)} virtual table hierarchy and (or) class hierarchy containing only legitimate targets for this callsite, and
\textit{6)} virtual table hierarchy and (or) class hierarchy of the whole program.
...
can basically call any possible entry in the class, vTable hierarchy or the whole program by not being constrained by any build-in check during
runtime. In order to achieve this the attacker corrupts an indirect function call, \ding{182}, 
...
We build our own function parameter count-based policy which resembles the policy introduced by TypeArmor~\cite{veen:typearmor} and
for this reason we opted to briefly present it herein.
Calltargets are classified based on the number of parameters that these provide and callsites are classified
by the number of parameters they these require. 
