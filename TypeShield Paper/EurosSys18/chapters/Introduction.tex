...
the compiler needs to generate a virtual table meta-data structure of all 
...
illegitimate forward-edge and backward-edge filtering. \textsc{TypeShield} backward edge policy is
based on the observation that backward-edges of a program can be precisely protected
if there is a precise forward-edge mapping in firrst place available.
...
to build a precise backward-edge policy which represents a good alternative along shadow stack based techniques.
...
Itanium C++ ABI~\cite{itanium:abi} x86-64 calling convention. The caller callee calling convention essentially means that every 
called function will return at the next address located after the callsite. This means that there is a one to one mapping between each caller and 
each callee. 
...
[leftmargin=.35cm]
 ...
 \item \textbf{Novel Technique.} We designed a novel fine-grained CFI technique for protecting forward and backward edges in a CFG
 against code reuse attacks.
 
 \item \textbf{Usable Prototype.} We implemented, \textsc{TypeShield}, a prototype which enforces the aforementioned technique
 ...
 \item \textbf{Experiments Results.} We conduct a thorough set of evaluative experiments in which we show that \textsc{TypeShield} is more precise and effective than 
...
%  against code reuse attacks. 
% We implemented, \textsc{TypeShield}, a prototype which enforces the forward and backward edge CFI policies 
