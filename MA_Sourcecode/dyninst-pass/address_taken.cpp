                                      ...
                                      CADecoder *decoder)
{
    ...
    DEBUG(LOG_FILTER_TAKEN_ADDRESS, "Processing object %s", object->name().c_str());
    ...
    TRACE(LOG_FILTER_TAKEN_ADDRESS, "TEXT(mem)  %lx -> %lx", text_start, text_end);
    ...
    TRACE(LOG_FILTER_TAKEN_ADDRESS, "PLT(mem)  %lx -> %lx", plt_start, plt_end);
...
// TODO: Only if we want to secure PLT and GOT (requires runtime module)
    ...
    auto const got_size = region->getMemSize();
        TRACE(LOG_FILTER_TAKEN_ADDRESS, "GOT(mem)  %lx -> %lx", got_start, got_end);

    if (!symtab->findRegion(region, ".data"))
        return false;
    auto const data_start = region->getMemOffset();
    auto const data_size = region->getMemSize();
        TRACE(LOG_FILTER_TAKEN_ADDRESS, "DATA(mem)  %lx -> %lx", data_start, data_end);
#endif

    if (!found_plt || !found_text)
    {
        ERROR(LOG_FILTER_TAKEN_ADDRESS, ".plt or .text section not found -> could not process");
    }
    else
    {
            ...
            if (symtab->findRegion(region, region_name))
            {
                ...
                for (auto value_ptr = region_data_begin; value_ptr < region_data_end; ++value_ptr)
                {
                    ...
                    auto type_fn = [plt_start, plt_end, value_offset,
                        ...
                        {
                            TRACE(LOG_FILTER_TAKEN_ADDRESS, "%lx:%lx,.plt", value_offset, value);
                            return ".plt";
                        }

                        if (region_name == ".rodata")
                        {
                            TRACE(LOG_FILTER_TAKEN_ADDRESS, "%lx:%lx,.data", value_offset, value);
                            return ".data";
                        }

                        TRACE(LOG_FILTER_TAKEN_ADDRESS, "%lx:%lx,%s", value_offset, value,
                              region_name);
                        return region_name;
                    };
                        ...
                        (plt_start <= value && value <= plt_end))
                    {
                        ...
                        if (type == ".plt")
                        {
                            taken_addresses[value].emplace_back(std::string("plt"), object,
                                                                value_offset);
                            DEBUG(LOG_FILTER_TAKEN_ADDRESS, "<PLT>%lx:%lx", value_offset, value);
                        }
                        ...
                        else if (is_function_start(image, value))
                        {
                            ...
                            DEBUG(LOG_FILTER_TAKEN_ADDRESS, "<DATA>%lx:%lx", value_offset, value);
                        }
                    }
                }
            }
...
// TODO: Only if we want to secure GOT and PLT (requires runtime module)
    ...
    */
    ...
    if (symtab->findRegion(region, ".rela.dyn"))
    {
        ...
        TRACE(LOG_FILTER_TAKEN_ADDRESS, ".rela.dyn %lu entries:", relocs.size());

        for (auto &&reloc : relocs)
        {
            TRACE(LOG_FILTER_TAKEN_ADDRESS, "\t tar: %lu rel: %lu add: %lu", reloc.target_addr(),
    ...
    if (symtab->findRegion(region, ".rela.plt"))
    {
        ...
        TRACE(LOG_FILTER_TAKEN_ADDRESS, ".rela.plt %lu entries:", relocs.size());
        for (auto &&reloc : relocs)
        {
            TRACE(LOG_FILTER_TAKEN_ADDRESS, "\t tar: %lu rel: %lu add: %lu", reloc.target_addr(),
                  reloc.rel_addr(), reloc.addend());
    ...
    instrument_object_instructions_unordered(
        object, [&](PatchBlock::Insns::value_type &instruction) {
            ...
            TRACE(LOG_FILTER_INSTRUMENTATION, "Processing instruction %lx : %lx", address,
                  deref_address);
            ...
            {
                DEBUG(LOG_FILTER_TAKEN_ADDRESS, "<PLT>%lx:%lx", address, deref_address);
...
template <> std::string to_string(TakenAddressSource &address_source)
{
    return "[ <" + address_source.source_name + "> " + address_source.object->name() + " @ " +
           int_to_hex(address_source.address) + "]";
}

template <> std::string to_string(TakenAddress &address)
{
    return "<AT> " + int_to_hex(address.first) + ": " + to_string(address.second);
}

template <> std::string to_string(TakenAddresses &addresses)
{
    std::stringstream ss;

    for (auto &&address : addresses)
        ss << to_string(address) << "\n";
