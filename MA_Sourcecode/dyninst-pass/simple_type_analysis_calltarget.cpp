...
template <> std::string to_string(std::pair<bool, simple_type_analysis::type_state_t> &state)
{
        ...
        target_state = std::accumulate(
            ...
            [](RegisterStates state, RegisterStates next) {
                return transform(state, next, [](state_t current, state_t delta) {
                    ...
                    // is this ok with unknown ? maybe fix afterwards with UNKNOWN == W0 ? is the
                    // locked par ok ?
        ...
        for (auto &instruction : insns)
        {
            if (change_possible)
            {
                ...
                /*                else if (decoder->is_constant_write())
                                {
                                    TRACE(LOG_FILTER_LIVENESS_ANALYSIS, "\tInstruction is a constant
                   write");
                   ...
                   state) {
                                    ...
                                    TRACE(LOG_FILTER_LIVENESS_ANALYSIS, "\t SANITIZED DECODER STATE
                   %s",
                                          to_string(register_states).c_str());
                ...
                else
                {
                    ...
                    {
                        TRACE(LOG_FILTER_TYPE_ANALYSIS, "%s",
                              operand.format(instruction_ptr->getArch()).c_str());
                    }

                    decoder->operand_information();
                    /*                    TRACE(LOG_FILTER_LIVENESS_ANALYSIS, "\t PREVIOUS STATE
                       %s",
                                              to_string(block_state).c_str());
                       ...
                       convert_states<state_t>(register_states);
                                        ...
                                        TRACE(LOG_FILTER_LIVENESS_ANALYSIS, "\t RESULT STATE %s",
                                              to_string(block_state).c_str());
                    */
                    change_possible = block_state.state_exists(std::make_pair(false, SIMPLE_TYPE_UNKNOWN)) || 
