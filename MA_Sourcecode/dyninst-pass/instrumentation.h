...
void instrument_image_objects(BPatch_image *image, instr_op_t instr_op, arg_ts &&... args)
{
    ...
    {
        // Instrument object
        TRACE(LOG_FILTER_INSTRUMENTATION, "Processing object %s", object->name().c_str());
...
void instrument_object_functions(BPatch_object *object, instr_op_t instr_op, arg_ts &&... args)
{
    ...
    for (auto module : modules)
    {
        ...
        TRACE(LOG_FILTER_INSTRUMENTATION, "Processing module%s: %s",
        ...
        for (auto function : functions)
        {
            ...
            TRACE(LOG_FILTER_INSTRUMENTATION, "Processing function %s", funcname);
                                               ...
                                               arg_ts &&... args)
{
    ...
    for (auto block : blocks)
    {
        ...
        TRACE(LOG_FILTER_INSTRUMENTATION, "Processing basic block %lx", block->getStartAddress());
                                        ...
                                        arg_ts &&... args)
{
    ...
    for (auto &instruction : insns)
    {
        ...
        TRACE(LOG_FILTER_INSTRUMENTATION, "Processing instruction %lx", address);
