...
ReachingRegisterStateMap reaching_init() { return {}; }

static ReachingRegisterStates merge_horizontal(std::vector<ReachingRegisterStates> states)
{
    ReachingRegisterStates target_state;
    if (states.size() > 0)
    {
        target_state = std::accumulate(
            ++(states.begin()), states.end(), states.front(),
            [](ReachingRegisterStates current, ReachingRegisterStates delta) {
                return transform(current, delta, [](reaching_state_t current,
                                                    reaching_state_t delta) {
                    if (current == REGISTER_SET && delta != REGISTER_SET)
                        current = REGISTER_TRASHED; // Check if it is possible to have
                                                    // REGISTER_UNKNOWN here
                    else if (current == REGISTER_UNKNOWN && delta == REGISTER_TRASHED)
                        current =
                            REGISTER_TRASHED; // consider both: REGISTER_TRASHED | REGISTER_SET }
                    return current;
                });
            });
    }
...
static ReachingRegisterStates merge_vertical(ReachingRegisterStates current,
                                             ReachingRegisterStates delta)
{
...
{
    std::vector<ReachingRegisterStates> states;

    for (auto block : blocks)
        states.push_back(reaching_analysis(decoder, image, block, block_states));
                                         ...
                                         ReachingRegisterStateMap &block_states)
{
    ...
    TRACE(LOG_FILTER_REACHING_ANALYSIS, "\tANALYZING FUNCTION %s [%lx:0x%lx[", funcname, start,
          end);
    TRACE(LOG_FILTER_REACHING_ANALYSIS, "\tFUN+----------------------------------------- %s",
          funcname);

    ReachingRegisterStates state;
        ...
        for (auto block : exit_blocks)
        {
            ...
            for (auto &instruction : insns)
            {
                ...
                if (decoder->is_return())
                    states.push_back(
                        reaching_analysis(decoder, image, block, address, block_states));
            }
        }
...
static void process_instruction(CADecoder *decoder,
                                ...
                                uint64_t end_address, ReachingRegisterStates &block_state,
                                         ...
                                         ReachingRegisterStateMap &block_states)
{
    ...
    auto itr_bool = block_states.insert({bb_start_address, ReachingRegisterStates()});
    auto &block_state = itr_bool.first->second;

    if (itr_bool.second)
    {
        TRACE(LOG_FILTER_REACHING_ANALYSIS, "\tBB+----------------------------------------- %lx",
              bb_start_address);
        ...
        if (change_possible)
        {
            ...
            TRACE(LOG_FILTER_REACHING_ANALYSIS, "Final merging for BasicBlock %lx",
                  bb_start_address);

            TRACE(LOG_FILTER_REACHING_ANALYSIS, "\t PREVIOUS STATE %s",
                  to_string(block_state).c_str());

            auto delta_state = reaching_analysis(decoder, image, sources, block_states);
...
{
    auto bb_start_address = block->getStartAddress();
    TRACE(LOG_FILTER_REACHING_ANALYSIS, "Processing History of BasicBlock %lx before reaching %lx",
          bb_start_address, end_address);

    TRACE(LOG_FILTER_REACHING_ANALYSIS,
          "BB_History+----------------------------------------- %lx : %lx", bb_start_address,
          end_address);

    ReachingRegisterStates block_state;
    PatchBlock::Insns insns;
    Dyninst::PatchAPI::convert(block)->getInsns(insns);
    ...
    if (change_possible)
    {
        ...
        TRACE(LOG_FILTER_REACHING_ANALYSIS, "Final merging for BasicBlock History %lx : %lx",
              bb_start_address, end_address);
