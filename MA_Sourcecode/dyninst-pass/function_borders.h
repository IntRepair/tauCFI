#ifndef __FUNCTION_BORDERS_H
#define __FUNCTION_BORDERS_H
...
{
void initialize(BPatch_object *object);
...
void apply(source_t *source, std::vector<BPatch_basicBlock *> &targets)
{
    targets.erase(std::remove_if(std::begin(targets), std::end(targets),
                                 [source](BPatch_basicBlock *target) {
                                     return !is_valid(source, target);
                                 }),
                  std::end(targets));
}

template <typename source_t>
void apply(source_t *source, std::set<BPatch_basicBlock *> &targets)
{
    for (auto itr = targets.begin(); itr != targets.end();)
    {
        if (!is_valid(source, *itr))
        {
