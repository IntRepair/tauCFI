...
void CADecoder::operand_information()
{
    ...
    for (auto pos = 0; pos < src_count; ++pos)
    {
        ...
        for (auto reg_index = 0; reg_index < regs; ++reg_index)
        {
            ...
            INFO(LOG_FILTER_TYPE_ANALYSIS, "Register %s READ [size: %d]", get_register_name(reg),
                 opnd_size_in_bits(reg_get_size(reg)));
        }
    }
    ...
    for (auto pos = 0; pos < dst_count; ++pos)
    {
        ...
        else
        {
            ...
            INFO(LOG_FILTER_TYPE_ANALYSIS, "Register %s WRITTEN [size: %d]", get_register_name(reg),
                 opnd_size_in_bits(reg_get_size(reg)));
        }
    }
    ...
    for (int reg = DR_REG_RAX; reg <= DR_REG_DIL; ++reg)
    {
        ...
        if (state != 0)
            INFO(LOG_FILTER_TYPE_ANALYSIS, "Register %s %s", get_register_name(reg),
