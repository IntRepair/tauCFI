...
template <std::size_t min, std::size_t max, typename state_t> struct __register_states_t
{
  ...
  public:
    ...
    template <typename binary_fun_t> friend this_t transform(this_t current, this_t delta, binary_fun_t binary_fun)
    {
...
__register_states_t<min, max, new_state_t> convert_states(__register_states_t<min, max, old_state_t> register_states)
{
    __register_states_t<min, max, new_state_t> conv_states;
    std::transform(register_states.begin(), register_states.end(), conv_states.begin(), [](old_state_t state) { return convert_state<new_state_t>(state); });
    return conv_states;
}
...
std::string to_string(__register_states_t<min, max, register_state_t> const &register_states)
{
    ...
    for (auto reg = min; reg <= max; ++reg)
    {
        ...
        ss << to_string(reg_state) << ((reg == max) ? ']' : ' ');
    }
