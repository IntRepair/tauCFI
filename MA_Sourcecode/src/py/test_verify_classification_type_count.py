    ...
    for test_target in test_config.configure_targets(argv):
        ...
        for opt in result_count.keys():
            ...
            if 0 == int(cs_count_dict["cs"]):
                cs_compound_dict["count cs args perfect"] = "-"
                cs_compound_dict["count cs args perfect pct"] = "-"
                cs_compound_dict["count cs args problem"] = "-"
                cs_compound_dict["count cs args problem pct"] = "-"
                cs_compound_dict["count cs non-void correct"] = "-"
                cs_compound_dict["count cs non-void correct pct"] = "-"
                cs_compound_dict["count cs non-void problem"] = "-"
                cs_compound_dict["count cs non-void problem pct"] = "-"
            else:
                cs_compound_dict["count cs args perfect"] = str(cs_count_dict["+0"])
                cs_compound_dict["count cs args perfect pct"] = gen_pct_string(float(cs_count_dict["+0"]) / float(cs_count_dict["cs"]))
                cs_compound_dict["count cs args problem"] = str(cs_count_dict["problems"])
                cs_compound_dict["count cs args problem pct"] = gen_pct_string(float(cs_count_dict["problems"]) / float(cs_count_dict["cs"]))
                cs_compound_dict["count cs non-void correct"] = str(cs_count_dict["non-void-ok"])
                cs_compound_dict["count cs non-void correct pct"] = gen_pct_string(float(cs_count_dict["non-void-ok"]) / float(cs_count_dict["cs"]))
                cs_compound_dict["count cs non-void problem"] = str(cs_count_dict["non-void-problem"])
                cs_compound_dict["count cs non-void problem pct"] = gen_pct_string(float(cs_count_dict["non-void-problem"]) / float(cs_count_dict["cs"]))

            if 0 == int(cs_type_dict["cs"]):
                cs_compound_dict["type cs args perfect"] = "-"
                cs_compound_dict["type cs args perfect pct"] = "-"
                cs_compound_dict["type cs args problem"] = "-"
                cs_compound_dict["type cs args problem pct"] = "-"
                cs_compound_dict["type cs non-void correct"] = "-"
                cs_compound_dict["type cs non-void correct pct"] = "-"
                cs_compound_dict["type cs non-void problem"] = "-"
                cs_compound_dict["type cs non-void problem pct"] = "-"
            else:
                cs_compound_dict["type cs args perfect"] = str(cs_type_dict["perfect"])
                cs_compound_dict["type cs args perfect pct"] = gen_pct_string(float(cs_type_dict["perfect"]) / float(cs_type_dict["cs"]))
                cs_compound_dict["type cs args problem"] = str(cs_type_dict["problems"])
                cs_compound_dict["type cs args problem pct"] = gen_pct_string(float(cs_type_dict["problems"]) / float(cs_type_dict["cs"]))
                cs_compound_dict["type cs non-void correct"] = str(cs_type_dict["non-void-ok"])
                cs_compound_dict["type cs non-void correct pct"] = gen_pct_string(float(cs_type_dict["non-void-ok"]) / float(cs_type_dict["cs"]))
                cs_compound_dict["type cs non-void problem"] = str(cs_type_dict["non-void-problem"])
                cs_compound_dict["type cs non-void problem pct"] = gen_pct_string(float(cs_type_dict["non-void-problem"]) / float(cs_type_dict["cs"]))
            ...
            if 0 == int(ct_count_dict["ct"]):
                ct_compound_dict["count ct args perfect"] = "-"
                ct_compound_dict["count ct args perfect pct"] = "-"
                ct_compound_dict["count ct args problem"] = "-"
                ct_compound_dict["count ct args problem pct"] = "-"
                ct_compound_dict["count ct void correct"] = "-"
                ct_compound_dict["count ct void correct pct"] = "-"
                ct_compound_dict["count ct void problem"] = "-"
                ct_compound_dict["count ct void problem pct"] = "-"
            else:
                ct_compound_dict["count ct args perfect"] = str(ct_count_dict["-0"])
                ct_compound_dict["count ct args perfect pct"] = gen_pct_string(float(ct_count_dict["-0"]) / float(ct_count_dict["ct"]))
                ct_compound_dict["count ct args problem"] = str(ct_count_dict["problems"])
                ct_compound_dict["count ct args problem pct"] = gen_pct_string(float(ct_count_dict["problems"]) / float(ct_count_dict["ct"]))
                ct_compound_dict["count ct void correct"] = str(ct_count_dict["void-ok"])
                ct_compound_dict["count ct void correct pct"] = gen_pct_string(float(ct_count_dict["void-ok"]) / float(ct_count_dict["ct"]))
                ct_compound_dict["count ct void problem"] = str(ct_count_dict["void-problem"])
                ct_compound_dict["count ct void problem pct"] = gen_pct_string(float(ct_count_dict["void-problem"]) / float(ct_count_dict["ct"]))

            if 0 == int(ct_type_dict["ct"]):
                ct_compound_dict["type ct args perfect"] = "-"
                ct_compound_dict["type ct args perfect pct"] = "-"
                ct_compound_dict["type ct args problem"] = "-"
                ct_compound_dict["type ct args problem pct"] = "-"
                ct_compound_dict["type ct void correct"] = "-"
                ct_compound_dict["type ct void correct pct"] = "-"
                ct_compound_dict["type ct void problem"] = "-"
                ct_compound_dict["type ct void problem pct"] = "-"
            else:
                ct_compound_dict["type ct args perfect"] = str(ct_type_dict["perfect"])
                ct_compound_dict["type ct args perfect pct"] = gen_pct_string(float(ct_type_dict["perfect"]) / float(ct_type_dict["ct"]))
                ct_compound_dict["type ct args problem"] = str(ct_type_dict["problems"])
                ct_compound_dict["type ct args problem pct"] = gen_pct_string(float(ct_type_dict["problems"]) / float(ct_type_dict["ct"]))
                ct_compound_dict["type ct void correct"] = str(ct_type_dict["void-ok"])
                ct_compound_dict["type ct void correct pct"] = gen_pct_string(float(ct_type_dict["void-ok"]) / float(ct_type_dict["ct"]))
                ct_compound_dict["type ct void problem"] = str(ct_type_dict["void-problem"])
                ct_compound_dict["type ct void problem pct"] = gen_pct_string(float(ct_type_dict["void-problem"]) / float(ct_type_dict["ct"]))
