...
#define LOG_INFO(filter, fmt, ...)                                                       \
    {                                                                                    \
        if ((LOG_LEVEL_CONFIG & LOG_LEVEL_INFO) == LOG_LEVEL_INFO)                       \
            if (filter & LOG_FILTER_CONFIG)                                              \
                std::printf("[LOG_INFO @" #filter "] " fmt "\n", ##__VA_ARGS__);         \
        if ((LOG_LEVEL_FILE_CONFIG & LOG_LEVEL_INFO) == LOG_LEVEL_INFO)                  \
            if (filter & LOG_FILTER_FILE_CONFIG)                                         \
                std::fprintf(FileLogger::log_file(),                                     \
                             "[LOG_INFO @" #filter "] " fmt "\n", ##__VA_ARGS__);        \
    }
