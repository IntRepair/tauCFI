...
template <> liveness::state_t convert_state(RegisterStateEx reg_state)
{
    ...
    liveness::state_t state = (((reg_state & REGISTER_EX_READ_64) == REGISTER_EX_READ_64)
    ...
    {
        state = (((reg_state & REGISTER_EX_WRITE_64) == REGISTER_EX_WRITE_64)
...
RegisterStates analysis(AnalysisConfig &config, BPatch_function *function)
{
    ...
    LOG_INFO(LOG_FILTER_GENERAL, "\tANALYZING FUNCTION %s [%lx:0x%lx[",
              funcname, start, end);
    ...
    {
        LOG_INFO(LOG_FILTER_GENERAL, "\tProcessing Entry Blocks");
                        ...
                        std::vector<BPatch_basicBlock *> path)
{
    ...
    LOG_INFO(LOG_FILTER_GENERAL, "Processing BasicBlock %lx",
              bb_start_address);
    ...
    if (itr_bool.second)
    {
        LOG_INFO(LOG_FILTER_GENERAL, "Information not in Storage");
        ...
        instrument_basicBlock_instructions(block, [&](Insns::value_type &instruction) {
            ...
            {
                LOG_INFO(LOG_FILTER_GENERAL, "\tIgnoring instruction %lx",
                          address);
                return;
            }
                ...
                (!config.follow_calls && decoder->is_call()))
            {
                ...
                LOG_INFO(LOG_FILTER_GENERAL, "\t RESULT STATE %s",
                          to_string(block_state).c_str());
            ...
            {
                LOG_INFO(LOG_FILTER_GENERAL, "\tInstruction is a direct call");
...
#if 1
            else if (config.follow_calls && decoder->is_possible_direct_tailcall())
            {
                LOG_INFO(LOG_FILTER_GENERAL,
                          "\tInstruction is possibly a direct tail call");
            ...
            {
                LOG_INFO(LOG_FILTER_GENERAL, "\tInstruction is a return");
            ...
            else if (decoder->is_constant_write())
            {
                ...
                LOG_INFO(LOG_FILTER_GENERAL, "\t SANITIZED DECODER STATE %s",
                          to_string(register_states).c_str());
            ...
            {
                LOG_INFO(LOG_FILTER_GENERAL, "\tProcessing instruction %lx",
                          address);
                ...
                auto source_reg = decoder->get_reg_source(0);
                auto target_reg = decoder->get_reg_target(0);
                if (source_reg && target_reg)
                {
                    if (decoder->is_move_to(*source_reg))
                    {
                        // LOG_INFO(LOG_FILTER_GENERAL, "\t MOVE ON THE SAME REGISTER %lx:
                        // DELTA STATE %s", decoder->get_address(),
                        // to_string(register_states).c_str());

                        register_states =
                            transform(register_states, [](RegisterStateEx state) {
                                return state &
                                       REGISTER_EX_READ; // Essentially remove all reads
                            });
                    }
                }
        ...
        if (change_possible)
        {
            ...
            {
                LOG_INFO(LOG_FILTER_GENERAL, "possible target address: %u",
                          target->getStartAddress());
            }

            function_borders::apply(block, targets);

            LOG_INFO(LOG_FILTER_GENERAL, "Final merging for BasicBlock %lx",
                      bb_start_address);
