...
def ret_type(entry):
	return entry["ret_type"][1]["wideness"]
...
def compiled_fn_precision(padyn, clang, cfn_acc_padyn_clang):
	...
	for key_pair in cfn_acc_padyn_clang:
		arg_type_padyn = arg_type(padyn_lookup[key_pair[0]])
		arg_type_clang = arg_type(clang_lookup[key_pair[1]])

		if arg_type_padyn == arg_type_clang:
			cfn_perfect_args += [key_pair]
		elif arg_type_padyn > arg_type_clang:
			cfn_problem_args += [(key_pair, padyn_lookup[key_pair[0]], clang_lookup[key_pair[1]])]

		ret_padyn = ret_type(padyn_lookup[key_pair[0]])
		ret_clang = ret_type(clang_lookup[key_pair[1]])

		if ret_padyn == ret_clang:
			cfn_perfect_ret += [key_pair]
		elif ret_padyn < ret_clang:
...
def call_site_precision(padyn, clang, cs_acc_padyn_clang):
	...
	for (key_pair, css_padyn, css_clang) in cs_acc_padyn_clang:
		for index, (cs_padyn, cs_clang) in enumerate(zip(css_padyn, css_clang)):
			count += 1
			arg_type_padyn = arg_type(cs_padyn)
			arg_type_clang = arg_type(cs_clang)
			...
			ret_padyn = ret_type(cs_padyn)
			ret_clang = ret_type(cs_clang)
...
class PrecisionTypePass(AdvancedPass):
	def __init__(self, config, args):
		AdvancedPass.__init__(self, "precision_type", config, args)
		fieldnames = ["opt"]
		fieldnames += ["target"]
		...
		config_prefixes = config["type_shield.pass.precision_type.prefixes"].split(" ")
		assert(len(config_prefixes) > 0)

		self.prefixes = [prefix + "type" for prefix in config_prefixes]
	...
	def _run_on_target(self, target, padyn_base, clang):
		...
		for prefix in self.prefixes:
			...
			self.log_errors_target(prg_name, [prefix + "CFN arg problematic CLANG[" + str(arg_type(clang_cfn)) + "] PADYN[" + str(arg_type(padyn_cfn)) + "] for [" + str(key_pair) + "]:\n\t" + str(padyn_cfn) + "\n\t" + str(clang_cfn) for (key_pair, padyn_cfn, clang_cfn) in cfn_problem_args])

			self.log_errors_target(prg_name, [prefix + "CFN ret problematic CLANG[" + str(ret_type(clang_cfn)) + "] PADYN[" + str(ret_type(padyn_cfn)) + "] for [" + str(key_pair) + "]:\n\t" + str(padyn_cfn) + "\n\t" + str(clang_cfn) for (key_pair, padyn_cfn, clang_cfn) in cfn_problem_ret])
			...
			self.log_errors_target(prg_name, [prefix + "CS arg problematic CLANG[" + str(arg_type(cs_clang)) + "] PADYN[" + str(arg_type(cs_padyn)) + "] for [" + str(key_pair) + "]:\n\t" + str(cs_padyn) + "\n\t" + str(cs_clang) for (key_pair, index, cs_padyn, cs_clang) in cs_problem_args])

			self.log_errors_target(prg_name, [prefix + "CS ret problematic CLANG[" + str(ret_type(cs_clang)) + "] PADYN[" + str(ret_type(cs_padyn)) + "] for [" + str(key_pair) + "]:\n\t" + str(cs_padyn) + "\n\t" + str(cs_clang) for (key_pair, index, cs_padyn, cs_clang) in cs_problem_ret])
