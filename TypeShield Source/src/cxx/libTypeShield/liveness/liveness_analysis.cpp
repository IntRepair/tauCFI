...
RegisterStates analysis(AnalysisConfig &config, BPatch_function *function)
{
    ...
    {
        function_borders::apply(function, entry_blocks);
    ...
    if (itr_bool.second)
    {
        ...
        instrument_basicBlock_instructions(block, [&](Insns::value_type &instruction) {
            ...
            else if (config.follow_calls && decoder->is_call())
            {
                ...
                {
                    auto range = decoder->get_register_range();
                    for (auto reg_index = range.first; reg_index <= range.second;
                         ++reg_index)
                    {
                        auto &reg_state = block_state[reg_index];
            ...
            else if (!decoder->is_nop())
            {
                ...
                if (source_reg && target_reg)
                {
                    ...
                    {
                        // LOG_TRACE(LOG_FILTER_LIVENESS_ANALYSIS, "\t MOVE ON THE SAME REGISTER %lx:
        ...
        if (change_possible)
        {
            ...
            {
                LOG_TRACE(LOG_FILTER_LIVENESS_ANALYSIS, "possible target address: %lx",
