...
def main(argv):
    ...
    for index in range(6):
        tag = str(index) + "count"
        csv_defs[0][0] = "classification_ct." + tag
        csv_defs[1][0] = "classification_cs." + tag
        csv_defs[2][0] = "classification_comp." + tag
        test_config.run_in_test_environment(argv, "classification_" + tag, csv_defs, (lambda argv, error_files, csv_writers: count_analysis(argv, error_files, csv_writers, tag)))


    for geomean_tag in geomean_dicts.keys():
        entry = geomean_dicts[geomean_tag]
