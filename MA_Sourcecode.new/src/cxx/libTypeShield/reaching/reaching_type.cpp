...
merge_horizontal_intersection_inter(std::vector<RegisterStates> states) {
  return impl::merge_horizontal(
      states, [](RegisterStates current, RegisterStates delta) {
        return transform(current, delta, [](state_t current, state_t delta) {
          if (is_trashed(delta) || is_trashed(current))
...
static RegisterStates
merge_horizontal_intersection_union(std::vector<RegisterStates> states) {
  return impl::merge_horizontal(
      states, [](RegisterStates current, RegisterStates delta) {
        return transform(current, delta, [](state_t current, state_t delta) {
          if (is_trashed(delta) || is_trashed(current))
...
std::vector<AnalysisConfig> init(CADecoder *decoder, ast::ast const &ast) {
  ...
  */
  ...
  init_config.merge_horizontal_rip = &merge_horizontal_union;
  ...
  init_config.block_callers = calculate_block_callers(init_config, ast);
  init_config.depth = 2;
  ...
  init_config.follow_into_callers = false;
  {
    AnalysisConfig config(init_config);
    config.follow_return_edges = false;
    config.merge_horizontal_intra = &merge_horizontal_intersection_inter;
