    ...
    for (auto &mbb : MF) {
      for (auto &MI : mbb) {
        ...
        if (is_call && !cs)
        {
          ...
          //for (auto const& operand : MI.operands())
          //  errs() << operand << " ";

         errs() << name << " test failed (is_call !is_cs)" << &MI << '\n';
        } else if (!is_call && cs){
          if (F)
            errs().write_escaped(F->getName()) << "  F";
          else
...
struct CallSiteDump_MachineFunction : public MachineFunctionPass {
  ...
  bool runOnMachineFunction(MachineFunction &MF) override {
    ...
    if (MF.getName() == "cv_undo") {
      for (auto &mbb : MF) {
        if (dump_type == IR) {
          if (auto bb = mbb.getBasicBlock()) {
            errs().write_escaped(MF.getName()) << " IR(BB) " << bb << *bb << "\n";
            for (auto &instr : *bb) {
              errs().write_escaped(MF.getName()) << "\t IR(INSTR) " << &instr << instr
                                                 << "\n";
            }
          }
        }
        ...
        if (dump_type == MR) {
          errs().write_escaped(MF.getName()) << " MR(BB) " << &mbb << mbb << "\n";
          for (auto &instr : mbb) {
            errs().write_escaped(MF.getName()) << "\t MR(INSTR) " << &instr << instr;
          }
        }
        errs() << '\n';
      }
