 ...
 * Copyright 1998-2011 Standard Performance Evaluation Corporation
 * All Rights Reserved
 *
 * specinvoke.c: The platform-independent bits
 *
 * $Id: specinvoke.c 6392 2011-03-26 00:00:00Z cloyce $
 */
...
static char *svnrev    = "$Revision: 6392 $";
static char *rcsid     = "$Id: specinvoke.c 6392 2011-03-26 00:00:00Z cloyce $";
...
int main (int argc, char *argv[], char *env[]) {
    ...
    int num_commands, num_copies, cur_copy, num_env, cur_env;
    char command[16384];
    ...
    /* Parse arguments */
    while ((c = my_getopt(argc, argv, "vnd:i:o:e:f:c:s:A?hErS:NZCD:qk")) != EOF) {
	switch (c) {
	    ...
	    case 'D': debug  = atoi(optarg);
                      ...
                      break;
	    case 'q': state.exit_on_first_error = 1; break;
	    case 'k': state.exit_on_first_error = 0; break;
	}
    }
    ...
    if (command_info == NULL) {
        printf ("Error allocating memory for command structures: %s (%d)\n",
                STRERROR(errno), errno);
        specinvoke_exit (1, &state);
    }
    copy_info = (copy_info_t *)malloc(num_copies * sizeof(struct copy_info_s));
    if (command_info == NULL) {
        printf ("Error allocating memory for copy_info structures: %s (%d)\n",
                STRERROR(errno), errno);
        specinvoke_exit (1, &state);
    }
    if (num_env) {
        state.env_vars = (char **)malloc((num_env + 1) * sizeof(char *));
        state.env_vals = (char **)malloc((num_env + 1) * sizeof(char *));
        if (state.env_vars == NULL || state.env_vals == NULL) {
            printf ("Error allocating memory for environment variables: %s (%d)\n",
                    STRERROR(errno), errno);
            specinvoke_exit (1, &state);
        }
	...
	while (*ptr == '-') {
		...
		*ptr++ = '\0';
		...
		case 'C': 
		    non_command_line = 1;
		    if (concurrent) {
			fprintf(stderr, "WARNING: Ignoring -C in command file in favor of -c on the command line.\n");
                        break;
                    }
		...
		case 'b': 
		    non_command_line = 1;
		    if (concurrent) {
			fprintf(stderr, "WARNING: Ignoring -b in command file in favor of -c on the command line.\n");
                        break;
                    }
		    if (cur_copy >= num_copies) {
		    ...
		    non_command_line = 1;
                    ...
                    state.env_vars[cur_env] = strdup(arg);
    ...
    if (!state.dry_run || state.dry_run >= 2) {
        if ((!state.dry_run && env == 0) || (state.dry_run >= 2 && state.env_vars == 0)) {
		...
		if (pid == copy_info[i].pid) {
			...
			fprintf (state.outfp,
				 ...
				 (long)pid, (unsigned int) status);
                     ...
                     * to start (obviously), and either there have been no
                     * previous child errors or exit_on_first_error (-q) isn't
                     * set.
                     */
		    if (  copy_info[cindex].index < num_commands
                        && (child_error == 0 || !state.exit_on_first_error)) {
