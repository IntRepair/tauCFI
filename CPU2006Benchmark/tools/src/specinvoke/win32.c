 ...
 *
 * $Id: win32.c 6490 2011-05-06 18:41:42Z cloyce $
 */

#include "specinvoke.h"

static char *versionid="1.0";
static char *rcsid="$Id: win32.c 6490 2011-05-06 18:41:42Z cloyce $";
...
pid_t invoke(copy_info_t *ui, command_info_t *ci, char **env,
    ...
    fprintf (si->outfp, "child started: %u, %lu, %lu, pid=%ld, '%s'\n", ui->num,
             (unsigned long)(ui->start_time.sec), (unsigned long)(ui->start_time.nsec), (long)(ui->pid), cmd);
...
long wait_for_next (long *status, int nowait) {
	...
	int errnum = GetLastError ();
	fprintf (stderr, "WaitForMultipleObjects had weird return code %lu! (%d)\n",
                 rc, errnum);
    }
    return pid;
}
...
long time_cal(void) {
     ...
     */
    #define TRIES 25
    struct _timeb time, prev;
    int i = 0, avg_count = 0, count, counts[TRIES];
    long avg_time = 0, deltas[TRIES];

    _ftime(&time); 
    prev.time = time.time;
    prev.millitm = time.millitm;

    for(i = 0; i < TRIES; i++) {
        count = 0;
	while (prev.millitm == time.millitm &&
	       prev.time  == time.time) {
            _ftime(&time); 
            count++;
	}
	deltas[i] = ((time.time - prev.time) * 1000) + (time.millitm - prev.millitm);
	counts[i] = count;
        prev.time = time.time;
        prev.millitm = time.millitm;
    }
    ...
    avg_time /= i;
    avg_count /= i;
    avg_time *= 1000000;	/* Make it nanoseconds */
    if (debug)
